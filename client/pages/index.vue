 ...
 <main class="bg-white p-10 min-h-screen">
   <div class="max-w-4xl mx-auto">
     <header class="flex justify-between items-center mb-20">
       ...
       <div>
        <Button>Lista</Button>
        <Button @click="toggleAddEmployeeDrawer">Add New Book</Button>
       </div>
     </header>
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full border-collapse border border-gray-300">
      ...
      <tbody>
        ...
        >
          ...
          <td class="border border-gray-300 p-2">{{ employee.document }}</td>
          <td class="border border-gray-300 p-2">{{ employee.birth_date }}</td>
   ...
   <EmployeeDrawerDialog
     v-if="isEmployeeDrawerOpen"
     :open="isEmployeeDrawerOpen"
     @update:open="(open: boolean) => isEmployeeDrawerOpen = open"
     :employee="currentEmployee"
     @refresh-data="fetchEmployees"
   />
   <DeleteEmployeeDrawerDialog
     v-if="isDeleteEmployeeDialogOpen"
     :open="isDeleteEmployeeDialogOpen"
     @update:open="(open: boolean) => isDeleteEmployeeDialogOpen = open"
     :employee="currentEmployee"
     @refresh-data="fetchEmployees"
   />
 </main>
